...
function updateClock() {
    ...
    if (d) d.innerText = `${now.getFullYear()}/${(now.getMonth()+1).toString().padStart(2,'0')}/${now.getDate().toString().padStart(2,'0')} (${days[now.getDay()]})`;
...
function initStickies() {
    ...
    stickies.forEach((s, i) => {
        ...
        d.innerHTML = `<textarea oninput="stickies[${i}].content=this.value; localStorage.setItem('sticky-notes',JSON.stringify(stickies));">${s.content}</textarea>
                       <span onclick="stickies.splice(${i},1); localStorage.setItem('sticky-notes',JSON.stringify(stickies)); initStickies();" style="position:absolute;top:0;right:5px;cursor:pointer;">×</span>`;
        c.appendChild(d);
    });
}
function addStickyNote() {
    stickies.push({color: document.getElementById('note-color').value, content: ""});
    localStorage.setItem('sticky-notes', JSON.stringify(stickies)); initStickies();
}
...
function saveIdeas() { localStorage.setItem('idea-pages', JSON.stringify(ideaPages)); initIdeas(); }
